 ...
 * user-submitted stories saved to localStorage, inspirational tone throughout
 */
import { useState } from "react";
import PageHeader from "@/components/PageHeader";
import PageFooter from "@/components/PageFooter";
import { Heart, Plus, X, Star, Quote, Users, ChevronDown, ChevronUp, Send } from "lucide-react";
...
function loadUserStories(): Story[] {
  try { return JSON.parse(localStorage.getItem("user-success-stories") || "[]"); } catch { return []; }
}

function saveUserStories(stories: Story[]) {
  try { localStorage.setItem("user-success-stories", JSON.stringify(stories)); } catch {}
}
...
export default function SuccessStories() {
  const [userStories, setUserStories] = useState<Story[]>(loadUserStories);
      ...
      <div className="px-4 py-5">
        ...
        <div className="flex items-center gap-2 mb-4">
          <div className="flex gap-1.5 flex-1">
            {([["all", `All Stories (${BUILT_IN_STORIES.length + userStories.length})`], ["community", `Community (${userStories.length})`]] as [typeof activeTab, string][]).map(([tab, label]) => (
        ...
        {showForm && (
          <div className="rounded-xl p-4 mb-5 fade-up" style={{ background: "white", border: "1.5px solid #C9922A", boxShadow: "0 4px 16px rgba(201,146,42,0.12)" }}>
            ...
            <p className="text-xs text-[#6B7280] mb-4 leading-relaxed" style={{ fontFamily: "'Source Sans 3', sans-serif" }}>
              Your story could be exactly what someone else needs to hear today. Share as much or as little as you're comfortable with.
            </p>
            <div className="flex flex-col gap-3">
              ...
              <div>
                ...
                <textarea className={`${inputClass} resize-none`} style={{ ...inputStyle, minHeight: "100px" }} placeholder="What challenge did you face? What did you do? What happened?" value={form.story} onChange={e => setForm(f => ({ ...f, story: e.target.value }))} />
              </div>
              <div>
            ...
            <div className="flex gap-3 mt-4">
              <button onClick={() => setShowForm(false)} className="flex-1 py-2.5 rounded-xl font-semibold text-sm text-[#6B7280]" style={{ background: "#F3F4F6", fontFamily: "'Source Sans 3', sans-serif" }}>Cancel</button>
              <button onClick={handleSubmit} className="flex-1 py-2.5 rounded-xl font-bold text-sm text-white flex items-center justify-center gap-2" style={{ background: "#C9922A", fontFamily: "'Source Sans 3', sans-serif" }}>
        ...
        {activeTab === "community" && userStories.length === 0 && (
              ...
              <div key={story.id} className="rounded-xl overflow-hidden fade-up" style={{ background: "white", border: "1.5px solid #E5E7EB", boxShadow: "0 3px 12px rgba(0,0,0,0.07)" }}>
                ...
                <div className="px-4 pt-4 pb-3">
                  <div className="flex items-start gap-3">
                    ...
                    {story.isUserSubmitted && (
                      <button onClick={() => handleDelete(story.id)} className="w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0" style={{ background: "#FEF2F2" }}>
                  ...
                  <div className="mt-3 flex items-start gap-2">
                    ...
                    <p className="text-sm text-[#2D2D2D] leading-relaxed italic line-clamp-3" style={{ fontFamily: "'Playfair Display', serif" }}>
